...
//					+ "bhit,bgroup,bstep,bindent from replyboard order by bid desc";
	...
	public boolean reply(int bid, String bname, String btitle, String bcontent, String bgroup, String bstep,
		...
		try {
			...
			System.out.println("rn1 : "+rn1);//0이면 댓글이 하나도 없을 떄
			...
			System.out.println("rn2 : "+rn2); // 현제 conn의 기본 commit 상태 확인
			System.out.println("commit 상태 : "+conn.getAutoCommit());
			if(rn1>=0 && rn2>=1) {
				conn.commit();
				System.out.println("커밋하기");
			}else {
				conn.rollback();
				System.out.println("롤백하기");
			}
	...
	public int replyShape(String bgroup, String bstep, Connection conn) {
		// db 연결 후 insert 쿼리 실행
		int flag = 0;
		PreparedStatement pstmt = null;
		try {
